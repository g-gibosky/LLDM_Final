        ...
        if (!$assertionsDisabled && !Thread.holdsLock(this.mLock)) {
            ...
            switch (1) {
                ...
                case 1:
                    ...
                    Log.m38w(TAG, "Low-memory device: shared RELROs used in all processes", new Object[0]);
                    break;
                case 2:
                    Log.m38w(TAG, "Beware: shared RELROs used in all processes!", new Object[0]);
                    this.mBrowserUsesSharedRelro = true;
                    break;
                default:
    ...
    public long getBaseLoadAddress() {
        ...
        synchronized (this.mLock) {
            ...
            if (this.mInBrowserProcess) {
                ...
                Log.m38w(TAG, "Shared RELRO sections are disabled in this process!", new Object[0]);
                j = 0;
            }
        }
        return j;
    ...
    private void setupBaseLoadAddressLocked() {
        if (!$assertionsDisabled && !Thread.holdsLock(this.mLock)) {
            ...
            if (this.mBaseLoadAddress == 0) {
                Log.m38w(TAG, "Disabling shared RELROs due address space pressure", new Object[0]);
                this.mBrowserUsesSharedRelro = false;
                this.mWaitForSharedRelros = false;
            }
        }
        ...
        if (!$assertionsDisabled && !Thread.holdsLock(this.mLock)) {
            ...
            for (Entry<String, LibInfo> entry : relroMap.entrySet()) {
                ...
                if (!nativeUseSharedRelro(libName, (LibInfo) entry.getValue())) {
                    Log.m38w(TAG, "Could not use shared RELRO section for " + libName, new Object[0]);
                }
            }
            if (!this.mInBrowserProcess) {
            ...
            if ($assertionsDisabled || this.mPrepareLibraryLoadCalled) {
                ...
                if (isFixedAddressPermitted && ((this.mInBrowserProcess && this.mBrowserUsesSharedRelro) || this.mWaitForSharedRelros)) {
                    ...
                    if (loadAddress > this.mBaseLoadAddress + 201326592) {
                        ...
                        Log.m28e(TAG, errorMessage, new Object[0]);
                ...
                if (NativeLibraries.sEnableLinkerTests) {
                    ...
                    Log.m29i(TAG, String.format(Locale.US, "%s: %s %x", new Object[]{tag, libFilePath, Long.valueOf(libInfo.mLoadAddress)}), new Object[0]);
                }
                if (this.mInBrowserProcess && !nativeCreateSharedRelro(sharedRelRoName, this.mCurrentLoadAddress, libInfo)) {
                    Log.m38w(TAG, String.format(Locale.US, "Could not create shared RELRO for %s at %x", new Object[]{libFilePath, Long.valueOf(this.mCurrentLoadAddress)}), new Object[0]);
                }
