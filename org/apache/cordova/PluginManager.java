...
public class PluginManager {
    ...
    public void exec(String service, String action, String callbackId, String rawArgs) {
        ...
        if (plugin == null) {
            LOG.m4d(TAG, "exec() call to unknown plugin: " + service);
        ...
        try {
            ...
            if (duration > ((long) SLOW_EXEC_WARNING_THRESHOLD)) {
                LOG.m16w(TAG, "THREAD WARNING: exec() call to " + service + "." + action + " blocked the main thread for " + duration + "ms. Plugin should use CordovaInterface.getThreadPool().");
            }
            if (!wasValidAction) {
            ...
            LOG.m8e(TAG, "Uncaught exception from plugin", e2);
