...
public class SlidingPaneLayout extends ViewGroup {
    ...
    static class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C04171());
        boolean isOpen;

        static class C04171 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C04171() {
            }
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.width == -2) {
                    ...
                    childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, PageTransition.CLIENT_REDIRECT);
                } else {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width, PageTransition.CLIENT_REDIRECT);
                }
                if (lp.height == -2) {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                } else if (lp.height == -1) {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, PageTransition.CLIENT_REDIRECT);
                } else {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(lp.height, PageTransition.CLIENT_REDIRECT);
                }
                ...
                if (child.getVisibility() != 8) {
                    ...
                    if (child.getVisibility() != 8) {
                        ...
                        if (!canSlide || child == this.mSlideableView) {
                            if (lp.weight > 0.0f) {
                                if (lp.width != 0) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(child.getMeasuredHeight(), PageTransition.CLIENT_REDIRECT);
                                } else if (lp.height == -2) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                                } else if (lp.height == -1) {
                                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, PageTransition.CLIENT_REDIRECT);
                                ...
                                if (canSlide) {
                                    ...
                                    childWidthSpec = MeasureSpec.makeMeasureSpec(newWidth, PageTransition.CLIENT_REDIRECT);
                                    if (measuredWidth != newWidth) {
                                        child.measure(childWidthSpec, childHeightSpec);
                                    }
                                } else {
                                    child.measure(MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), PageTransition.CLIENT_REDIRECT), childHeightSpec);
                                }
                            }
                            ...
                            child.measure(MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, PageTransition.CLIENT_REDIRECT), childHeightSpec);
                        }
                    }
                }
            }
